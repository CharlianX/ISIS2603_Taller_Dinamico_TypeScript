...
async function cargarBanda(url) {
    try {
        const respuesta = await fetch(url);
        const banda = await respuesta.json();
        const canciones = banda.topSongs.map((s) => new Song(s.name, s.album, s.plays, s.duration, s.year));
        return new Band(banda.name, banda.debutYear, banda.formationYear, banda.members, banda.membersNames, banda.albums, banda.description, banda.imageUrl, canciones);
    }
    catch (error) {
...
function renderBanda(banda) {
    ...
    document.getElementById('imageUrl').src = banda.imageUrl;
...
function crearEntradaCancion(song) {
